...
export default {
  ...
  methods: {
	...
	this.ruleForm.zixunwenti = this.ruleForm.zixunwenti.replace(reg,'../../../springbootc90g5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zixunguzhu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zixunguzhuCrossAddOrUpdateFlag = false;
						 this.parent.search();
